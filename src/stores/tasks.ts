...
interface TasksState {
  ...
  getCountTasksByStatus: (status?: TaskStatus) => number;
...
export const useTasksStore = create<TasksState>((set, get) => ({
  ...
  getTasksByStatus: (status?: TaskStatus) => {
    if (status === undefined) {
        return get().tasks; // если статус не указан - все задачи
    }
    return get().tasks.filter(task => task.status === status);
  },

  getCountTasksByStatus: (status?: TaskStatus) => {
    if (status === undefined) {
        return get().tasks.length; // если статус не указан - все задачи
    }
...
getTaskById: (id) => {
  ...
  fetchTasks: async (params?: TasksQueryParams) => {
    ...
    console.log('🟢 После получения тасков:', tasks);
